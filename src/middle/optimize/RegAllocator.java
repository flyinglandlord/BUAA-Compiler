...
public class RegAllocator {
    ...
    public void buildConflictGraph(Function function) {
        for (BasicBlock block : function.getBasicBlocks()) {
            ...
            for (int i = block.getBlock().size() - 1; i >= 0; --i) {
                ...
                for (Symbol symbol1 : getAllUse(code)) {
                    if (!symbol1.isGlobal()) {
                        for (Symbol symbol2 : getAllUse(code)) {
                            if (!symbol2.isGlobal() && !symbol1.equals(symbol2)) {
                                conflictGraph.addEdge(symbol1, symbol2);
                            }
                        }
                        conflictGraph.addSymbol(symbol1);
                        out.add(symbol1);
                    }
                }
    ...
    public void allocateSSA(Function function) {
        ...
        System.out.println("Saved registers: " + savedRegList);
        System.out.println("Local registers: " + localRegList);
    }
}
