...
import frontend.grammar.decl.const_decl.ConstExpInitVar;
...
public class CalcConstExpr {
    private final SymbolTable symbolTable;
    private final ErrorTable errorTable;

    public CalcConstExpr(SymbolTable symbolTable, ErrorTable errorTable) {
    ...
    public int calcUnaryExp(UnaryExp exp) throws ConstExpException {
        ...
        for (int i = exp.getOperators().size() - 1; i >= 0; i--) {
    ...
    public int calcLVal(LVal lVal) throws ConstExpException {
        ...
        if (!symbol.isConst()) throw new ConstExpException(lVal.getIdent().getLinenumber(), name);

        if (!symbol.isArray()) {
            if (symbol instanceof FunctionFormParam) throw new ConstExpException(lVal.getIdent().getLinenumber(), name);
            else if (symbol instanceof Variable) {
                return ((Variable) symbol).getCalcConstInitVal().get(0);
            }
        } else {
            if (symbol instanceof FunctionFormParam) throw new ConstExpException(lVal.getIdent().getLinenumber(), name);
            ...
            return ((Variable) symbol).getCalcConstInitVal().get(offset);
        }
        return 0;
    }
}
